...
TOKEN = "8903937011:AAFMEg13ckLOLfBvEIjwPoi4BJo5YVjYpdk"
...
def analyze_stock(ticker_symbol):
    ...
    try:
        ...
        # 1. Penanganan MultiIndex jika ada
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = df.columns.get_level_values(0)

        # 2. Hapus kolom duplikat agar df['Close'] selalu menghasilkan Series tunggal
        df = df.loc[:, ~df.columns.duplicated()]

        # 3. Hapus semua baris data yang harganya 0 atau kosong (NaN)
